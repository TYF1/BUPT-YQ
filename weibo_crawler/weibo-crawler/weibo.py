...
class Weibo(object):
    ...
    def get_json(self, params):
        ...
        # TODO 打印json的info
        # logger.info("获取json的信息：url：{}; json:{}".format(url, r.json()))
    ...
    def get_user_info(self):
        ...
        global times
        if times != 0:
            sleep(random.randint(70, 180))  # 睡了多久？
        times += 1
    ...
    def get_long_weibo(self, id):
        ...
        for i in range(5):
            ...
            # TODO：打印长微博的INFO
            # logger.info("长微博: url:{}; html:{}; js:{}".format(url, html, js))
            if weibo_info:
                weibo = self.parse_weibo(weibo_info)
                # logger.info("长微博: weibo: ", weibo)
                return weibo
    ...
    def get_pages(self):
        ...
        try:
            ...
            if self.get_user_info() != 0:
                return
            logger.info("准备搜集 {} 自从{}关于{}的微博".format(self.user["screen_name"],
...
if __name__ == "__main__":
    ...
    # 建立已有用户集合
    # users = set()
    # with open("E:\\user\\Desktop\\面向领域2\\weibo_crawler\\weibo-crawler\\users.txt", mode="r", encoding="utf-8") as fp:
    #     for l in fp.readlines():
    #         l = l.split(" ")[0]
    #         users.add(int(l))
    # for i in range(1):
    # with open("users.txt", mode="a", encoding="utf-8") as fp:
    #     try:
    #         cur.execute("select user_id from weibo")
    #         data = cur.fetchall()
    #         for e in data:
    #             id = int(e.get("user_id"))
    #             if id not in users:
    #                 users.add(id)
    #                 fp.write(e.get("user_id") + "\n")
    #                 # print(id)
    #     except pmq.DatabaseError as e:
    #         print(e)
    # print("---------------start " + str(i) + " time-----------------")
    main()
    print("total_count {}".format(total_count))
    # print("--------------- end " + str(i) + " time -----------------")
