...
public interface comInfoMapper {
    ...
    @Insert("INSERT INTO tb_info(cid,date_time,lat,lon,user_id) values(#{cid},#{date_time},#{lat},#{lon},#{user_id})")
    boolean insertSomeData(@Param("cid")int cid,@Param("date_time")Date date_time,@Param("lat") double lat,@Param("lon") double lon,@Param("user_id")int use_id);
}
