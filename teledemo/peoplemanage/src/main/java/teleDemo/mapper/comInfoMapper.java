...
public interface comInfoMapper {
    ...
    @Select("select lat, lon from eqe.tb_info where user_id = #{id};")
    @Results(id="effectedLatLon",value={
            @Result(column = "lat",property = "lat",jdbcType = JdbcType.DOUBLE),
            @Result(column = "lon",property = "lon",jdbcType = JdbcType.DOUBLE),
    })
    List<tbInfo> getEffectedLatLon(Integer id);

    @Select("select lat, lon from eqe.tb_info where user_id = #{id};")
    @Results(id="closeContactsLatLon",value={
            @Result(column = "lat",property = "lat",jdbcType = JdbcType.DOUBLE),
            @Result(column = "lon",property = "lon",jdbcType = JdbcType.DOUBLE),
    })
    List<tbInfo> getCloseContactsLatLon(Integer id);
