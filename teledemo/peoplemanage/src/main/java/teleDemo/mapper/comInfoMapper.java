...
public interface comInfoMapper {
    ...
    @Select("select * from teledata.tb_info where lac=#{lac};")
    @ResultMap(value = "tbInfoMap")
    @Transactional
    List<tbInfo> getInfoByLac(@Param("lac") String lac);

    @Select("select * from teledata.tb_info where user_id=#{userId};")
    @ResultMap(value = "tbInfoMap")
    @Transactional
    List<tbInfo> getInfoById(int userId);
